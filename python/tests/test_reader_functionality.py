...
def run_tests(mdl):
    ...
    try:
        ...
        assert len(session_data.request) < 10000, f"Request suspiciously large: {len(session_data.request)}"

        # Validate BLE identifier
        assert isinstance(session_data.ble_ident, bytes), "BLE identifier should be bytes"
        assert len(session_data.ble_ident) == 16, f"BLE identifier should be 16 bytes, got {len(session_data.ble_ident)}"
        ...
        assert doc_request.doc_type == "org.iso.18013.5.1.mDL", f"Wrong doc type: {doc_request.doc_type}"
        ...
        assert auth_status == mdl.AuthenticationStatus.VALID, f"Expected VALID auth, got {auth_status}"
