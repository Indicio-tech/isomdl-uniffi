...
def run_tests():
    ...
    try:
        ...
        try:
            reconstructed_mdl = mdl.Mdoc.from_stringified_document(cbor_str, key_pair.key_alias)
            ...
            if original_doctype == reconstructed_doctype:
                ...
                print(f"   ❌ Document types don't match: {original_doctype} vs {reconstructed_doctype}")
                return False
                
        except Exception as e:
            print(f"   ⚠️  MDL reconstruction test failed (expected for some formats): {e}")
