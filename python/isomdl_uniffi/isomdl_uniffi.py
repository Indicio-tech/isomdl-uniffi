...
def _uniffi_check_api_checksums(lib):
    ...
    if lib.uniffi_isomdl_uniffi_checksum_method_mdoc_key_alias() != 9235:
        raise InternalError("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    if lib.uniffi_isomdl_uniffi_checksum_method_mdoc_status() != 4157:
        raise InternalError("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    ...
    if lib.uniffi_isomdl_uniffi_checksum_constructor_mdlpresentationsession_new() != 1270:
        raise InternalError("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    if lib.uniffi_isomdl_uniffi_checksum_constructor_mdoc_create_and_sign() != 40803:
        raise InternalError("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    if lib.uniffi_isomdl_uniffi_checksum_constructor_mdoc_create_and_sign_mdl() != 62480:
        raise InternalError("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    ...
    if lib.uniffi_isomdl_uniffi_checksum_constructor_p256keypair_new() != 41043:
        raise InternalError("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    if lib.uniffi_isomdl_uniffi_checksum_constructor_preparedmdoc_new() != 3758:
        raise InternalError("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    if lib.uniffi_isomdl_uniffi_checksum_constructor_preparedmdoc_new_mdl() != 22621:
        raise InternalError("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
...
_UniffiLib.uniffi_isomdl_uniffi_fn_method_mdoc_status.argtypes = (
    ctypes.c_void_p,
    ctypes.POINTER(_UniffiRustCallStatus),
)
_UniffiLib.uniffi_isomdl_uniffi_fn_method_mdoc_status.restype = _UniffiRustBuffer
...
_UniffiLib.uniffi_isomdl_uniffi_checksum_method_mdoc_status.argtypes = (
)
_UniffiLib.uniffi_isomdl_uniffi_checksum_method_mdoc_status.restype = ctypes.c_uint16
...
class IssuerVerificationResult:
    ...
    def __init__(self, *, verified: "bool", common_name: "typing.Optional[str]", error: "typing.Optional[str]"):
        self.verified = verified
        self.common_name = common_name
        self.error = error

    def __str__(self):
        return "IssuerVerificationResult(verified={}, common_name={}, error={})".format(self.verified, self.common_name, self.error)
...
class _UniffiConverterTypeIssuerVerificationResult(_UniffiConverterRustBuffer):
    @staticmethod
    def read(buf):
        return IssuerVerificationResult(
            verified=_UniffiConverterBool.read(buf),
            common_name=_UniffiConverterOptionalString.read(buf),
            error=_UniffiConverterOptionalString.read(buf),
        )

    @staticmethod
    def check_lower(value):
        _UniffiConverterBool.check_lower(value.verified)
        _UniffiConverterOptionalString.check_lower(value.common_name)
        _UniffiConverterOptionalString.check_lower(value.error)

    @staticmethod
    def write(value, buf):
        _UniffiConverterBool.write(value.verified, buf)
        _UniffiConverterOptionalString.write(value.common_name, buf)
...
# __dict__.  All of this happens in dummy class to avoid polluting the module
# namespace.
class MdocVerificationError(Exception):
    """
...
_UniffiTempMdocVerificationError = MdocVerificationError

class MdocVerificationError:  # type: ignore
    """
    Error type for issuer signature verification.
    """

    class X5ChainMissing(_UniffiTempMdocVerificationError):
        def __init__(self):
            pass

        def __repr__(self):
            return "MdocVerificationError.X5ChainMissing({})".format(str(self))
    _UniffiTempMdocVerificationError.X5ChainMissing = X5ChainMissing # type: ignore
    class X5ChainParsing(_UniffiTempMdocVerificationError):
        def __init__(self, *values):
            if len(values) != 1:
                raise TypeError(f"Expected 1 arguments, found {len(values)}")
            if not isinstance(values[0], str):
        ...
        def __repr__(self):
            return "MdocVerificationError.X5ChainParsing({})".format(str(self))
    _UniffiTempMdocVerificationError.X5ChainParsing = X5ChainParsing # type: ignore
    class TrustAnchorRegistryError(_UniffiTempMdocVerificationError):
        def __init__(self, *values):
            if len(values) != 1:
                raise TypeError(f"Expected 1 arguments, found {len(values)}")
            if not isinstance(values[0], str):
        ...
        def __repr__(self):
            return "MdocVerificationError.TrustAnchorRegistryError({})".format(str(self))
    _UniffiTempMdocVerificationError.TrustAnchorRegistryError = TrustAnchorRegistryError # type: ignore
    class X5ChainValidationFailed(_UniffiTempMdocVerificationError):
        def __init__(self, *values):
            if len(values) != 1:
                raise TypeError(f"Expected 1 arguments, found {len(values)}")
            if not isinstance(values[0], str):
        ...
        def __repr__(self):
            return "MdocVerificationError.X5ChainValidationFailed({})".format(str(self))
    _UniffiTempMdocVerificationError.X5ChainValidationFailed = X5ChainValidationFailed # type: ignore
    class IssuerAuthFailed(_UniffiTempMdocVerificationError):
        def __init__(self, *values):
            if len(values) != 1:
                raise TypeError(f"Expected 1 arguments, found {len(values)}")
            if not isinstance(values[0], str):
        ...
        def __repr__(self):
            return "MdocVerificationError.IssuerAuthFailed({})".format(str(self))
    _UniffiTempMdocVerificationError.IssuerAuthFailed = IssuerAuthFailed # type: ignore

MdocVerificationError = _UniffiTempMdocVerificationError # type: ignore
del _UniffiTempMdocVerificationError


class _UniffiConverterTypeMdocVerificationError(_UniffiConverterRustBuffer):
    @staticmethod
    def read(buf):
        variant = buf.read_i32()
        if variant == 1:
            return MdocVerificationError.X5ChainMissing(
            )
        if variant == 2:
            return MdocVerificationError.X5ChainParsing(
                _UniffiConverterString.read(buf),
            )
        if variant == 3:
            return MdocVerificationError.TrustAnchorRegistryError(
                _UniffiConverterString.read(buf),
            )
        if variant == 4:
            return MdocVerificationError.X5ChainValidationFailed(
                _UniffiConverterString.read(buf),
            )
        if variant == 5:
            return MdocVerificationError.IssuerAuthFailed(
                _UniffiConverterString.read(buf),
            )
        raise InternalError("Raw enum value doesn't match any cases")
    ...
    @staticmethod
    def check_lower(value):
        if isinstance(value, MdocVerificationError.X5ChainMissing):
            return
        if isinstance(value, MdocVerificationError.X5ChainParsing):
            _UniffiConverterString.check_lower(value._values[0])
            return
        if isinstance(value, MdocVerificationError.TrustAnchorRegistryError):
            _UniffiConverterString.check_lower(value._values[0])
            return
        if isinstance(value, MdocVerificationError.X5ChainValidationFailed):
            _UniffiConverterString.check_lower(value._values[0])
            return
        if isinstance(value, MdocVerificationError.IssuerAuthFailed):
            _UniffiConverterString.check_lower(value._values[0])
            return
...
class Mdoc():
    ...
    def _make_instance_(cls, pointer):
        ...
        return inst
    @classmethod
    def create_and_sign(cls, doc_type: "str",namespaces: "dict[str, dict[str, str]]",holder_jwk: "str",iaca_cert_perm: "str",iaca_key_perm: "str",status: "typing.Optional[str]"):
        ...
        _UniffiConverterOptionalString.check_lower(status)
        ...
        _UniffiConverterOptionalString.lower(status))
        return cls._make_instance_(pointer)

    @classmethod
    def create_and_sign_mdl(cls, mdl_items: "str",aamva_items: "typing.Optional[str]",holder_jwk: "str",iaca_cert_pem: "str",iaca_key_pem: "str",status: "typing.Optional[str]"):
    ...
    def status(self, ) -> "typing.Optional[str]":
        ...
        return _UniffiConverterOptionalString.lift(
            _uniffi_rust_call(_UniffiLib.uniffi_isomdl_uniffi_fn_method_mdoc_status,self._uniffi_clone_pointer(),)
        )
...
class PreparedMdoc():
    ...
    _pointer: ctypes.c_void_p
    def __init__(self, doc_type: "str",namespaces: "dict[str, dict[str, str]]",holder_jwk: "str",signature_algorithm: "str",status: "typing.Optional[str]"):
        """
    ...
    def _make_instance_(cls, pointer):
        ...
        return inst
    @classmethod
    def new_mdl(cls, mdl_items: "str",aamva_items: "typing.Optional[str]",holder_jwk: "str",signature_algorithm: "str",status: "typing.Optional[str]"):
        """
...
def verify_oid4vp_response(response: "bytes",nonce: "str",client_id: "str",response_uri: "str",trust_anchor_registry: "typing.Optional[typing.List[str]]",use_intermediate_chaining: "bool") -> "MdlReaderVerifiedData":
    _UniffiConverterBytes.check_lower(response)
    
    _UniffiConverterString.check_lower(nonce)
    
    _UniffiConverterString.check_lower(client_id)
    
    _UniffiConverterString.check_lower(response_uri)
    ...
    return _UniffiConverterTypeMdlReaderVerifiedData.lift(_uniffi_rust_call_with_error(_UniffiConverterTypeMdlReaderSessionError,_UniffiLib.uniffi_isomdl_uniffi_fn_func_verify_oid4vp_response,
        ...
        _UniffiConverterString.lower(response_uri),
        _UniffiConverterOptionalSequenceString.lower(trust_anchor_registry),
        _UniffiConverterBool.lower(use_intermediate_chaining)))
