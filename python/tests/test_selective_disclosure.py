...
def test_basic_selective_disclosure():
    ...
    assert result.device_authentication == mdl.AuthenticationStatus.VALID, "Device auth should be valid"
    ...
    assert disclosed_attrs == expected_attrs, f"Response should only contain {expected_attrs}, got {disclosed_attrs}"
...
def test_age_verification_attributes():
    ...
    age_permitted_items = {
        "org.iso.18013.5.1.mDL": {
            "org.iso.18013.5.1": ["age_over_18", "age_over_21"]
        }
    }
    ...
    assert age_18_item.is_bool(), f"age_over_18 should be boolean MDocItem, got: {type(age_18_item)}"
    assert age_21_item.is_bool(), f"age_over_21 should be boolean MDocItem, got: {type(age_21_item)}"
...
def test_minimal_disclosure():
    ...
    minimal_request = {
        "org.iso.18013.5.1": {
            "document_number": True  # Only request document number
        }
    }
    ...
    assert iso_namespace["document_number"] == True, "document_number should be required"
